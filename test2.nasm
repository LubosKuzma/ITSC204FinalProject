...
;unfountunately I couldn't figure out how to get it to work.
gnome_sort:
mov rsi, 0x101
mov rdi, rec_buffer
mov r8, [rsi] ; move size of array to r8
mov r9, 1 ; set index to 1

.loop:
cmp r9, r8 ; compare index to size of array
jge .done ; if index >= size of array, exit loop


mov r10, [rdi+r9*8-8] ; move array[i-1] to r10
mov r11, [rdi+r9*8] ; move array[i] to r11
cmp r10, r11 ; compare array[i-1] and array[i]
jle .increment ; if array[i-1] <= array[i], increment index

mov r12, r11 ; move array[i] to r12
mov [rdi+r9*8], r10 ; move array[i-1] to array[i]
mov [rdi+r9*8-8], r12 ; move r12 (array[i]) to array[i-1]
sub r9, 1 ; decrement index
